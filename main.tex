...
\setsansfont{HarmonyOS Sans}[AutoFakeSlant=0.2]
\setmonofont{CodeNewRoman Nerd Font}
\setCJKmainfont{HarmonyOS Sans SC}[AutoFakeSlant=0.2]
\setCJKsansfont{HarmonyOS Sans SC}[AutoFakeSlant=0.2]
\setCJKmonofont{HarmonyOS Sans SC}[AutoFakeSlant=0.2]
% --------------- 字体设置 --------------- %
...
\begin{frame}{最后试试伪代码}
  ...
  \scriptsize % 字太大了调小点
  \begin{algorithmic}[1]
    \setlength{\itemsep}{0.2em} % 调整行距
    % tex-fmt: off
    \State 设定环境参数、DRL超参数，随机初始化DQN参数，赋予target网络相同的参数

    \For{each episode}
      \State 重置环境

      \For{each step}
        \State 对于状态 $s_t$ 根据 $\epsilon$-greedy 策略得到动作 $a_t$
        \State 执行动作 $a_t$ 后环境变为 $s_{t+1}$ 并得到奖励 $r_t$
        \State 将轨迹 $(s_t, a_t, r_t, s_{t+1})$ 存入replay memory

        \If{step $>$ 开始学习步数}
          \State 从replay memory中随机抽取 $30$ 个样本作为 minibatch

          \For{each sample in minibatch}
            \State 将 $s_t$ 传入Q-network得到 $a_t$ 对应的 $Q_{value}$
            \State 将 $s_{t+1}$ 传入target-network得到输出的最大值 $Q_{target}$
            \State 根据损失函数 $Loss = (Q_{value} - (r_t + \gamma \cdot Q_{target}))^2$ 使用梯度下降法更新Q-network
          \EndFor

          \If{step \% 50 = 0}
            \State 使用Q-network的参数更新target-network
          \EndIf
